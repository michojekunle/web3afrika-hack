...
export default function App() {
  ...
  const { writeContract: createEvent, data: createEventData } =
    useWriteContract();
  ...
  const { writeContract: toggleEventStatus, data: toggleEventStatusData } =
    useWriteContract();
  ...
  const handleCreateEvent = () => {
    if (createEvent) {
      createEvent({
        ...
        args: [
          eventName,
          BigInt(new Date(eventDate).getTime() / 1000),
          nftAddress,
          BigInt(maxCapacity),
        ],
      });
      toast({
        ...
        description:
          "Your event is being created. Please wait for confirmation.",
      });
    }
  };
  ...
  const handleRegister = () => {
    if (register && eventId) {
      ...
      toast({
        title: "Registering",
        description:
          "Your registration is being processed. Please wait for confirmation.",
      });
    }
  };
  ...
  const handleToggleStatus = () => {
    if (toggleEventStatus && eventId) {
      ...
      toast({
        ...
        description:
          "The event status is being updated. Please wait for confirmation.",
      });
    }
  };
          ...
          <div className="space-y-4">
            ...
            {eventDetails && (
              <div className="mt-4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg">
                <p>
                  <strong>Name:</strong> {eventDetails[0]}
                </p>
                <p>
                  <strong>Date:</strong>{" "}
                  {new Date(Number(eventDetails[1]) * 1000).toLocaleString()}
                </p>
                <p>
                  <strong>Max Capacity:</strong> {eventDetails[2].toString()}
                </p>
                <p>
                  <strong>Registered Count:</strong>{" "}
                  {eventDetails[3].toString()}
                </p>
                <p>
                  <strong>Status:</strong>{" "}
                  {eventDetails[4] ? "Active" : "Inactive"}
                </p>
                {isOwner === address && (
                  <div className="mt-2">
