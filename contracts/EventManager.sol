...
contract EventManager {
    ...
    // @improvement: How can we prevent the Users from sharing the NFTs
    // to each other and registering, how do we restrict this flaw
    ...
    function updateEvent(
        ...
        validEvent.eventType = _eventType;
        validEvent.maxNumberOfRegistrations = _newMaxRegistrations;

        emit Events.EventUpdatedSuccessfully(
            _newEventName,
            msg.sender,
            _newNftAddress
        );
    ...
    function endEvent(uint256 _eventId) external {
        ...
        // check if event is a valid one
        EventObj storage validEvent = eventObjects[_eventId];
        validEvent.isDone = true;
        
        emit Events.EventEndedSuccessfully(
